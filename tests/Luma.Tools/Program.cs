...
if (mode == "index-benchmark")
{
    ...
    var options = new Luma.Server.Features.Indexing.IndexingOptions
    {
        ProcessingWorkers = count,
        CachePath = Path.Combine(run, "cache"),
    ...
    {
        media,
        processingWorkers = options.ProcessingWorkers,
        ...
        discoverySeconds = discovery,
        thumbnailsDoneSeconds = thumbnails,
        keywordsDoneSeconds = tags,
        mediaPerSecond = (photos + videos) / (thumbnails - discovery),
        cacheBytes,
        cacheBytesPerItem = cacheBytes / Math.Max(1, photos + videos),
...
if (mode == "decode-benchmark")
{
    ...
    var report = JsonSerializer.Serialize(new
    {
        ...
        width = 1920,
        height = 1080,
        processPerImageSeconds = cold.Elapsed.TotalSeconds,
        reusedProcessSeconds = reused.Elapsed.TotalSeconds,
        reusedImagesPerSecond = iterations / reused.Elapsed.TotalSeconds
    }, new JsonSerializerOptions { WriteIndented = true });
...
if (mode is "gate7-benchmark" or "gate7-traversal" or "gate7-queries")
{
    ...
    if (mode == "gate7-benchmark")
        foreach (var selective in new[] { false, true })
        {
            ...
            var samples = new List<double>(); for (var i = 0; i < 1000; i++) { var timer = Stopwatch.StartNew(); await db.QuerySingleOrDefaultAsync<long?>(sql, p); samples.Add(timer.Elapsed.TotalMilliseconds); }
            samples.Sort();
