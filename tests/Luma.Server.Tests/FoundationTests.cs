...
public sealed class FoundationTests
{
    ...
    public async Task Fresh_database_is_ready_and_restart_preserves_identity()
    {
        ...
        Assert.Equal(new StatusResponse("ready", 2), status);
        ...
        Assert.Equal(2, await connection.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM SchemaMigrations"));
    ...
    public async Task Cancelled_migration_does_not_change_the_database()
    {
        ...
        Assert.Equal(new StatusResponse("ready", 2), await client.GetFromJsonAsync<StatusResponse>("/api/status"));
    }
}
