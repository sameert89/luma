...
public sealed class FoundationTests
{
    ...
    public async Task Fresh_database_is_ready_and_restart_preserves_identity()
    {
        ...
        Assert.Equal(new StatusResponse("ready", 21, StatusEndpoints.Release), status);
        ...
        Assert.Equal(21, await connection.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM SchemaMigrations"));
    ...
    public async Task Cancelled_migration_does_not_change_the_database()
    {
        ...
        Assert.Equal(new StatusResponse("ready", 21, StatusEndpoints.Release), await client.GetFromJsonAsync<StatusResponse>("/api/status"));
    }
}
