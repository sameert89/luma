...
The million-row final-code run was completed on 2026-09-18 after the earlier attempt was interrupted; until then `stage7-final-queries-1m.json` held a copy of the pre-index report and is now replaced by [the real run](performance/stage7-final-queries-1m.json). Its first attempt aborted on the second combination when a broad tag-filtered query exceeded the 2 s deadline with the 1.45 GiB fixture evicted from the operating system page cache by concurrent build and container work; the file was read back into the page cache and the run repeated, without weakening the deadline. All 48 combinations then completed: worst p95 337.971 ms and worst p99 351.552 ms, both on selective type sorts; 20 selective combinations sit between 150 ms and 338 ms. Migration 11 is visible in the broad date-grouped sorts, which drop from roughly 270–290 ms to 1.3–1.5 ms p95; selective grouped combinations moved up by about 40 ms against the earlier report, within this machine's run-to-run spread. The cold-cache abort is itself a finding: a million-row broad tag query can exceed the deadline when the database is not resident, which the prescribed cold-trial release measurements must cover.
