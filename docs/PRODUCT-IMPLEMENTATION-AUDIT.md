...
| Collections destination | The requested feedback describes Collections as albums, folders, tags, and favourites. The destination is currently labelled Saved and renders only liked media. Folder and tag workflows exist elsewhere; a consolidated Collections destination is incomplete against that feedback. User-created albums are not sufficiently specified to assume a new album-management feature. | `USER_FEEDBACK.md`: primary destinations; `App.tsx`: `collections`, `galleryFilters` |
