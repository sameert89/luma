...
`POST /api/folders/{id}/scans` accepts `StartScanRequest` and rescans a folder and everything beneath it (a library's root folder: a full library scan). It returns 202 with `ScanAccepted`, 400 for an unknown metadata mode, 404 for an unknown folder or disabled library, and 409 while another scan of the library is queued or running. Recursive folder scans (migration 0017, `Scans.Recursive`) reconcile missing media only inside the folder's subtree. Folder-demand indexing (`POST /api/folders/{id}/index`) stays direct-only.
