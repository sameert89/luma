...
`groupBy=tag` is a presentation of tag collections rather than duplicated media cursors. `GET /api/collections/tag-groups` accepts shared media filters plus optional nonnegative afterId, returning up to 50 `{id,name}` tags and nullable nextId. Each tag must have at least one matching media item. Open a collection with the additive `collectionTag` exact-tag filter; it is ANDed with the original predicate, including any/all selected-tag matching. `/api/media` rejects groupBy=tag with 400: select a collection and use ordinary media cursor pagination. Multi-tag items occur in each applicable collection.
