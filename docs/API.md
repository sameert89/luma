...
| grouping | Persist date-day key; `(groupKey,sortKey,id)` for measured combinations. Rare combinations may sort the narrowed SQLite result under deadline; benchmark before release. Tag groups walk `Tags` in normalized-name order, one `(tagId,mediaId)` EXISTS per candidate tag, and read each group's media through that same semi-join |
...
`GET /api/settings/source-verification` returns `{ "enabled": false }`. `PUT /api/settings/source-verification` accepts the same shape and persists the server-wide preference. The setting defaults to disabled. Enabling it allows the bounded source-presence worker to check known paths between scans; disabling it prevents those batches from querying SQLite or touching source paths. Explicit scans continue to reconcile missing media.
...
Media summaries include nullable `groupKey` and `groupLabel`. Clients merge adjacent equal keys across page boundaries. Date grouping uses the stored UTC effective-date day; type grouping puts images before videos; tag grouping uses `tag:{id}` keys with the tag name as the label. Sort direction applies within each group. Name sorting uses normalized binary keys; captured sorting uses effective date with its documented fallback.
...
`GET /api/jobs/{id}/content` streams completed exports as attachments. Downloads expire after 24 hours and share a separate 1 GiB quota, checked against uncompressed output and final archive bytes. Quota exhaustion is visible as `job_quota_exceeded`; missing/unfinished/expired downloads are 404. Queued jobs survive restart. Running exports fail as `interrupted` and partial files are discarded. Running imports return to the queue and resume from durable per-file checkpoints with their counts retained. These operations never rewrite embedded metadata, write adjacent source sidecars or delete originals.
...
## Stage 8 additions (v1.0.2)

`POST /api/libraries/{id}/scans` additionally accepts `metadataMode: none|embedded|xmp` (default embedded). The selected mode is retained for subsequent folder-demand/startup scans. Scan enqueue and metadata-stage enqueue commit together; a full metadata queue returns 429 and rolls back the start. Index-only scans create no metadata job. The metadata worker waits until discovery for the library finishes, then works outside indexing transactions. Startup/recovery scans get a deduplicated durable stage. Metadata stages share the configured aggregate processing semaphore with cache workers and wait behind pending viewport-priority preparation.

Imports now include `found`, `updated`, and `skipped` file counts in job status. `processed` counts checkpointed attempts; `found` counts files with recognized keyword metadata; `updated` counts files gaining tag assignments; `skipped` counts unchanged automatic sources or successful attempts without new assignments. Findings/failures use `failed`, including invalid tags that may still merge valid assignments. These counts can overlap (found is not an outcome); they do not promise that every processed file was updated. Successful item codes include imported, no_metadata, unchanged; source_changed reports a source/XMP mutation during extraction and leaves its revision unfinished for a later import. Manual requests force extraction and can repair removed assignments. Internal automatic requests use `automatic: true` and a nullable scanId. Per-item tags, revision, checkpoint and counters commit atomically. Query imports resume after lastMediaId; selected imports skip existing checkpoints. Automatic revision keys combine indexed source revision, size/mtime and optional adjacent XMP sizes/mtimes, including additions/removals. No fingerprint is refreshed after a failed extraction.

`GET /api/tasks` returns at most 100 recent metadata and scan operations with string ID, kind, state, creation time, scope label, processed/failed/pending counts. Scan traversal completion remains distinct from cache preparation: a completed traversal with pending cache work is reported as running in this task view. Partial import findings appear in failure counts. This read uses only SQLite.

`groupBy=tag` groups `/api/media` results under their tags like every other grouping: one header per tag that has a matching item, in tag-name order, with the group's media following it and a multi-tag item repeated under each of its tags. Because an item can repeat, clients deduplicate by group key and ID together rather than by ID alone. Neighbors place an item in the first of its tags; an untagged item has no neighbors under this grouping. `GET /api/collections/tag-groups` remains available for listing the tags themselves: it accepts shared media filters plus optional nonnegative afterId, returning up to 50 `{id,name}` tags and nullable nextId, each with at least one matching media item. The additive `collectionTag` exact-tag filter narrows results to one tag; it is ANDed with the original predicate, including any/all selected-tag matching.

`GET /api/media/{id}/progress` returns `{mediaId,positionSeconds,durationSeconds,watchedSeconds,state,updatedAt}`, with unwatched defaults for a video without saved progress. Non-video/unknown records return 404. `PUT` accepts finite nonnegative positionSeconds (at most duration), positive durationSeconds, and watchedSeconds as a playback-time increment from 0 to 30 seconds; invalid ranges return 400. In progress requires at least min(5 seconds, 20% duration) of actual playback. Completion requires position >=95% and accumulated playback >=min(30 seconds, 80% duration). Once completed, replaying does not remove completion. Progress is instance-wide since Luma has no per-user account model. The last committed position wins across devices. Media summaries include nullable watchProgress through one bounded batch query. Clients save about every ten seconds and on pause/ended/exit, excluding seeking and buffering time; abrupt process loss can lose the last interval. Seeking alone does not add watch time. Normal viewers offer resume/start-over; Reels shares progress recording without interrupting navigation with a prompt.
...
`POST /api/folders/{id}/scans` accepts `StartScanRequest` and rescans a folder and everything beneath it (a library's root folder: a full library scan). It returns 202 with `ScanAccepted`, 400 for an unknown metadata mode, 404 for an unknown folder or disabled library, and 409 while another scan of the library is queued or running. With `prioritize: true` — what refreshing the folder in view sends — a queued or running scan that already covers the folder (a library scan, that folder's own scan, or a recursive scan of an ancestor) is joined instead: the folder is moved to the front of that scan's discovery, its previews are prepared first, and the response is 202 with the running scan's ID. Anything the running scan will not reach still returns 409, as does a request that does not ask to prioritize. With `shallow: true` — also what refreshing the folder in view sends — the scan reads that one directory and nothing below it, and a library root stays a folder scan instead of becoming a library scan; its tag import and its missing-media reconciliation cover the same single folder. Recursive folder scans (migration 0017, `Scans.Recursive`) reconcile missing media only inside the folder's subtree. Folder-demand indexing (`POST /api/folders/{id}/index`) stays direct-only.

`GET /api/libraries` includes each library's `metadataMode`. `PUT /api/libraries/{id}/metadata-mode` with `{ "metadataMode": "none|embedded|xmp" }` changes it without starting any work: 204, 400 for an unknown mode, 404 for an unknown or disabled library. Scans that start through `POST /api/libraries/{id}/scans` or `POST /api/folders/{id}/scans` still store their mode on the library, as before.

`POST /api/libraries/{id}/root` records a never-scanned library's root folder so it can be browsed without a full scan, and returns 200 `{ "folderId": … }` (the existing root when there is one; 404 for an unknown or disabled library). It starts no work: each folder, the root included, is then indexed direct-only by `POST /api/folders/{id}/index` when first opened.

When `POST /api/folders/{id}/index` finds the folder already indexed (204) and the library's mode is not `none`, it also queues a direct-only automatic tag import for the folder if any present media directly inside has never been attempted by an import with at least the library's sidecar setting (no `MetadataRevisions` row and no `MetadataJobItems` row from such an import; migration 0019 indexes `MetadataJobItems(MediaId)` for this check). It queues nothing when a queued/running import already covers the folder, and skips silently when the metadata queue is full instead of returning 429. This fills in tags for folders indexed while the library imported none, or whose scan's metadata stage was cancelled.

`GET /api/search/suggestions?q=&limit=` (limit 1–20, default 8) returns `[{kind,label,id,detail,libraryId}]`: up to four `tag` completions by prefix on the tag key (only tags that label media), up to three `folder` completions (prefix on `IX_Folders_Name`, then from three characters substring over at most 200 `FolderSearch` trigram candidates; `detail` is the library and parent path, `libraryId` is set; library roots and hidden folders are excluded), then `file` completions by indexed name prefix and, from three characters, by name substring over at most 200 trigram-index candidates. `detail` is a file's folder path. Missing media and hidden folders are excluded; no directory enumeration or file access occurs. Empty `q` returns `[]`; `q` over 200 characters or an out-of-range limit returns 400.
...
Omitting both retains the existing ID-ascending API contract. `sort=modified` (migration 0018) orders by the directory's own modified time, which scans record from the traversal that already lists each directory (no extra file access). Folders not yet rescanned carry the newest direct file's time instead, and 0 when that is unknown. Name sorting now uses a stored natural sort key (`SortKey`: case and accent insensitive, digit runs by value) on `IX_Folders_Parent_Sort`; modified sorting uses `IX_Folders_Parent_Modified`. Name cursors issued before migration 0018 are rejected with `invalid_cursor`. The UI requests `modified` for the date sorts (modified, captured) and `name` for every other media sort. Name sorting uses SQLite
NOCASE over sibling paths with an ID tie-breaker and an indexed composite cursor; no
original-directory stats or enumeration occurs. Cursors bind parent, sort and direction;
changing those invalidates the cursor. The UI requests name sorting and shares the
visible sort direction (default descending) with media. Date, type, size and shuffle
