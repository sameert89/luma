...
Opening an incompletely indexed folder queues direct-folder discovery through a separate POST while the library is idle. For an indexed folder with refresh-on-open enabled, that POST reads one directory timestamp and queues the same shallow scan only when it changed. During a full-library scan, the POST instead hands bounded advisory priority to the owning worker while retaining its 409 response. The worker discovers direct media and child-folder entries using the existing bounds. Successful standalone reconciliation is restricted to that folder; unrelated folders remain untouched. Paused pending previews can resume on a later visit. See [the API contract](API-FOLDER-INDEXING.md), [original decision](adr/0007-folder-demand-indexing.md), [active-scan priority decision](adr/0008-active-scan-folder-priority.md), and [automatic refresh decision](adr/0011-automatic-library-refresh.md).

## Automatic discovery

Each library exposes three change-detection modes: filesystem watcher (default), periodic full scan, and manual only. Watcher events are coalesced per affected folder, debounced, checked for stable size/mtime and persisted as dirty-folder hints before the existing scan worker handles them. A new or moved directory is discovered from its parent and then only that subtree is recursively indexed. The periodic mode is an alternative for mounts whose event forwarding is unreliable; it does not run alongside the watcher. Refresh-on-open, watcher debounce, file-stability wait and periodic interval are independently configurable in Settings → Libraries.

Watcher events may be missed while Luma is stopped, when a platform notification buffer overflows, or when a Docker/network sharing layer fails to forward them. Refresh-on-open catches changes in visited folders; Rescan library remains the definitive reconciliation operation. Watchers perform no polling and no disk reads while the filesystem is idle.
