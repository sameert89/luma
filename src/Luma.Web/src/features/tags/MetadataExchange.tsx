...
export function MetadataExchange({ mediaIds, filters, dislikes = false, imports = true, exports = true, heading = true }: { mediaIds?: number[]; filters?: Filters; dislikes?: boolean; imports?: boolean; exports?: boolean; heading?: boolean }) {
  ...
  const start = useMutation({ mutationKey: ['background-task'], mutationFn: (kind: 'tags' | 'xmp' | 'dislikes') => request<components['schemas']['JobAccepted']>(`/api/${kind === 'tags' ? 'imports' : 'exports'}/${kind}`, undefined, 'POST', { mediaIds, query: filters, includeSidecars: sidecars }), onSuccess: result => { setJobId(result.id); setAfterMediaId(undefined); setResultPages([]); void client.invalidateQueries({ queryKey: ['tasks'] }) } })
  ...
  return <section aria-label="Metadata exchange" className="space-y-3">
    {heading && <h2 className="text-sm font-semibold">Metadata exchange</h2>}<p className="text-sm text-muted">{canImport ? 'Imports merge EXIF/XMP tags into Luma. ' : ''}{exports ? 'Exports are downloads; originals stay unchanged.' : 'Originals stay unchanged.'}</p>
    ...
    {job.data && <p role="status" className="text-sm">{job.data.state}: {job.data.processed} processed, {job.data.found ?? 0} with metadata, {job.data.updated ?? 0} imported/updated, {job.data.skipped ?? 0} skipped, {job.data.failed} failed.{job.data.failureCode && ` ${job.data.failureCode}`}</p>}
    ...
    {(afterMediaId || job.data?.nextMediaId) && <div className="flex flex-wrap gap-2">{afterMediaId && <QuietButton disabled={job.isFetching} onClick={() => { setAfterMediaId(resultPages.at(-1)); setResultPages(pages => pages.slice(0, -1)) }}>Previous item results</QuietButton>}{job.data?.nextMediaId && <QuietButton disabled={job.isFetching} onClick={() => { setResultPages(pages => [...pages, afterMediaId]); setAfterMediaId(job.data!.nextMediaId!) }}>Next item results</QuietButton>}</div>}
