...
const sameTag = (a: string, b: string) =>
  a.trim().normalize('NFC').toUpperCase() === b.trim().normalize('NFC').toUpperCase()
...
export function TagEditor({
  ...
  const suggestions = useQuery({
    queryKey: ['tags', typed],
    queryFn: ({ signal }) => request<Tag[]>(`/api/tags?prefix=${encodeURIComponent(typed)}&limit=8`, signal),
  ...
  const update = useMutation({
    mutationFn: async ({ id, remove = false }: { id?: number; remove?: boolean }) => {
      ...
      await request('/api/media/tags', undefined, 'POST', {
        mediaIds,
        addTagIds: remove ? [] : [id],
        removeTagIds: remove ? [id] : [],
      })
    },
    onSuccess: async () => {
      setName('')
      ...
      await Promise.all([
        ...
        client.invalidateQueries({ queryKey: ['detail'] }),
        client.invalidateQueries({ queryKey: ['neighbors'] }),
  ...
  function choose(tag: Tag) {
    if (!applied.has(tag.id)) update.mutate({ id: tag.id, remove: removing })
  }
  ...
  return (
    <section className="space-y-4" aria-label={bulk ? 'Bulk tag editor' : 'Tag editor'}>
      {!bulk && (
        <div className="flex flex-wrap gap-2">
          ...
          {tags.map(tag => (
            <QuietButton
              key={tag.id}
              className="tag-tone gap-2"
              style={tagTone(tag.name)}
              disabled={update.isPending}
              onClick={() => update.mutate({ id: tag.id, remove: true })}
      ...
      >
        ...
        <Field label={removing ? 'Tag to remove' : 'Find or create a tag'}>
          <div className="relative">
            ...
            <Input
              ...
              placeholder={removing ? 'Type a tag on these items' : 'Type to search, Enter to add'}
              value={name}
        ...
        {showResults && (
          ...
          >
            {matches.map(tag => {
              ...
              return (
                <li key={tag.id} className="border-b border-line last:border-b-0">
                  <button
                    type="button"
                  ...
                  >
                    <TagIcon className="size-4 shrink-0 text-accent" aria-hidden="true" />
                    <span
                      className="tag-tone min-w-0 flex-1 truncate rounded-full border px-2 py-1"
                      style={tagTone(tag.name)}
                    >
                      {tag.name}
                    </span>
            ...
            {creatable && (
              <li>
                ...
                >
                  ...
                  <span className="min-w-0 flex-1 truncate">Create “{name.trim()}”</span>
                  <span className="shrink-0 text-xs text-muted">New tag</span>
                </button>
              </li>
            )}
          </ul>
        ...
        {bulk && (
          <Button type="submit" disabled={update.isPending || !name.trim() || mediaIds.length === 0} className="gap-2">
            <Plus className="size-4" aria-hidden="true" />
            {update.isPending ? 'Saving…' : `Apply to ${mediaIds.length} items`}
          </Button>
        )}
      </form>
      {update.isPending && !bulk && (
