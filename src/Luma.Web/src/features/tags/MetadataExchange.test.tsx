...
it('makes import findings beyond the first hundred item results reachable', async () => {
  const fetch = vi.fn().mockImplementation((url: string) => Promise.resolve(new Response(JSON.stringify(url === '/api/imports/tags'
    ? { id: 1 }
    : { id: 1, kind: 'import', state: 'completed', processed: 120, found: 119, updated: 118, skipped: 1, failed: 1, items: url.includes('afterMediaId=100') ? [{ mediaId: 120, code: 'source_unavailable' }] : [{ mediaId: 100, code: 'imported' }], nextMediaId: url.includes('afterMediaId=100') ? null : 100 }), { headers: { 'Content-Type': 'application/json' } })))
  ...
  expect(await screen.findByRole('status')).toHaveTextContent('120 processed, 119 with metadata, 118 imported/updated, 1 skipped, 1 failed')
  expect(screen.queryByRole('button', { name: 'First item results' })).not.toBeInTheDocument()
  ...
  await userEvent.click(screen.getByRole('button', { name: 'Previous item results' }))
