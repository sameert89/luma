...
it('makes import findings beyond the first hundred item results reachable', async () => {
        ...
        JSON.stringify(
          url === '/api/imports/tags'
            ...
            : {
                ...
                skipped: 1,
                failed: 1,
                items: url.includes('afterMediaId=100')
                  ? [{ mediaId: 120, code: 'source_unavailable' }]
                  : [{ mediaId: 100, code: 'imported' }],
                nextMediaId: url.includes('afterMediaId=100') ? null : 100,
              },
        ),
  ...
  const { unmount } = render(
    <QueryClientProvider client={client}>
      <MetadataExchange mediaIds={Array.from({ length: 120 }, (_, index) => index + 1)} />
    </QueryClientProvider>,
  )
  await userEvent.click(screen.getByRole('button', { name: 'Import metadata tags' }))
  expect(await screen.findByRole('status')).toHaveTextContent(
    '120 processed, 119 with metadata, 118 imported/updated, 1 skipped, 1 failed',
  )
