...
export function RescanButton({ libraryId, name = 'library' }: { libraryId: number; name?: string }) {
  ...
  return <>
    <QuietButton aria-label={busy ? `View scan progress for ${name}` : `Rescan ${name}`} onClick={() => setOpen(true)}><RefreshCw className={`size-4 ${busy ? 'motion-safe:animate-spin' : ''}`} /><span className={busy ? 'text-xs' : 'sr-only'}>{action.isPending ? action.variables ? 'Cancelling…' : 'Starting…' : running ? 'Scanning' : preparing ? 'Preparing' : 'Rescan'}</span></QuietButton>
    <Modal open={open} onOpenChange={setOpen} title={busy ? 'Library scan progress' : `Rescan ${name}?`} description="Review the scan before starting it." sheet>
      <div className="space-y-4 overflow-auto p-5">
        ...
        <div className="flex gap-3">{busy ? <Button disabled={action.isPending} onClick={() => action.mutate(true)}>{action.isPending && action.variables ? 'Cancelling…' : 'Cancel scan'}</Button> : <Button disabled={action.isPending} onClick={() => action.mutate(false)}>Start rescan</Button>}<QuietButton onClick={() => setOpen(false)}>{busy ? 'Keep browsing' : 'Not now'}</QuietButton></div>
