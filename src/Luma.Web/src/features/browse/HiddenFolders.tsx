...
export function HiddenFolders() {
  ...
  return <SettingsSection title="Hidden folders" description="Hidden folders and everything inside them are left out of the library, search, Reels, slideshows and indexing. Hide a folder from its folder actions.">
    {hidden.isPending && <p role="status" className="text-sm text-muted">Loading hidden folders…</p>}
    {hidden.isError && <p role="alert" className="text-sm text-danger">{errorMessage(hidden.error)}</p>}
    {hidden.data?.length === 0 && <p className="text-sm text-muted">No folders are hidden.</p>}
    {!!hidden.data?.length && <ul className="divide-y divide-line">{hidden.data.map(folder => <li key={folder.id} className="flex items-center gap-3 py-2">
    ...
    {show.isError && <p role="alert" className="text-sm text-danger">{errorMessage(show.error)}</p>}
  </SettingsSection>
}
