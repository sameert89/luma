...
afterEach(() => { setCoverRenderingMode('smart'); vi.unstubAllGlobals() })

it('draws automatic covers in the Settings style, switching live, while a manual cover stays one photo', async () => {
  const { container, rerender } = render(<><AlbumCoverSettings /><AlbumCover images={images} height="h-24" fallback={null} /></>)
  ...
  rerender(<><AlbumCoverSettings /><AlbumCover images={[{ url: '/chosen', width: 180, height: 320 }]} override height="h-24" fallback={null} /></>)
  ...
  expect(screen.getByText(/A cover you picked yourself for an album or folder always stays that photo/)).toBeInTheDocument()
})
...
it('offers Reset album cover only for a manual cover and clears just that choice', async () => {
  ...
  const folder = { id: 7, libraryId: 1, parentId: 1, name: 'Trips', coverUrl: '/cover-1', coverOverride: false, coverImages: images } as FolderPage['current']
