...
export function MediaActions({
  ...
  preference: string
  onPreference: (value: 'liked' | 'disliked' | 'neutral') => void
  menuOpen: boolean
  ...
  return (
    <div
      data-chrome
      className={`pointer-events-none absolute right-3 z-10 flex flex-col items-end gap-2 transition-opacity duration-200 sm:right-5 ${faded ? 'opacity-0' : ''} ${className}`}
    >
      <div id={menuId} className={`motion-drop ${interactive}`} data-open={menuOpen} inert={!menuOpen}>
        ...
        <div className="flex max-h-[calc(100dvh-15rem)] min-h-0 flex-col items-end gap-2 overflow-y-auto overflow-x-hidden pb-0.5 no-scrollbar">
          {children}
        </div>
      </div>
      <div className={`flex items-center gap-2 ${interactive}`}>
        <IconButton
          ...
          onClick={() => onPreference(liked ? 'neutral' : 'liked')}
        >
        ...
        <IconButton
          ...
          className={actionButton}
          onClick={() => onPreference(disliked ? 'neutral' : 'disliked')}
        >
          <HeartCrack className={`size-4 ${disliked ? 'text-danger' : ''}`} />
        </IconButton>
        <IconButton
          label={menuOpen ? `Close ${menuLabel.toLowerCase()}` : menuLabel}
          aria-expanded={menuOpen}
          aria-controls={menuId}
          className={actionButton}
          onClick={() => onMenuOpenChange(!menuOpen)}
        >
          {menuOpen ? <X className="size-4" /> : <EllipsisVertical className="size-4" />}
        </IconButton>
      </div>
    </div>
  )
