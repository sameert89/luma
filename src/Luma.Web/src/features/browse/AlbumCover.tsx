...
import {
  coverLayout,
  foregroundBox,
  setCoverRenderingMode,
  useCoverRenderingMode,
  type CoverImage,
  type CoverRenderingMode,
} from './coverLayout'
...
function useCardSize() {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      if (width > 0 && height > 0) setSize({ aspect: width / height, width })
    }
    measure()
...
function Picture({ image, className = '', style }: { image: CoverImage; className?: string; style?: CSSProperties }) {
  ...
  return (
    <img
      ...
      loading="lazy"
      decoding="async"
      className={`${className} ${failed ? 'invisible' : ''}`}
      style={style}
...
export function AlbumCover({
  ...
  images?: readonly CoverImage[] | null
  override?: boolean
  height: string
  fallback: ReactNode
}) {
  ...
  return (
    <div
      ref={box}
      data-cover={layout?.kind === 'mosaic' ? `mosaic-${layout.tiles.length}` : layout?.fit}
      className={`relative overflow-hidden bg-canvas ${height}`}
    >
      {!layout ? (
        fallback
      ) : layout.kind === 'mosaic' ? (
        <div
          className={`grid h-full gap-0.5 ${layout.tiles.length === 3 ? 'grid-cols-[2fr_1fr]' : 'grid-cols-[2fr_1fr_1fr]'} grid-rows-2`}
        >
          {layout.tiles.map((image, index) => (
            <Picture
              key={image.url}
              image={image}
        ...
        <>
          ...
          <Picture
            image={layout.image}
            className={`absolute inset-0 h-full w-full scale-110 object-cover blur-lg ${layout.fit === 'blurred-foreground' ? 'brightness-50' : 'brightness-75'}`}
          />
          <Picture
            image={layout.image}
            className="absolute max-w-none shadow-lg"
            style={(() => {
...
const modes: Array<{ id: CoverRenderingMode; name: string; description: string; icon: ReactNode }> = [
  {
    id: 'smart',
    name: 'Smart album covers',
    icon: <LayoutGrid className="size-5" />,
    description:
      'Albums with three or more photos show a mosaic. Single covers fill the card, cropping gently, and photos far from the card’s shape sit enlarged over a blurred backdrop.',
  },
  {
    id: 'cropped',
    name: 'Cropped album covers',
...
export function AlbumCoverSettings() {
  ...
  return (
    <SettingsSection
      title="Album covers"
      description="Choose how Luma draws automatic album and folder covers on this device."
    >
      <div className="grid gap-3 sm:grid-cols-2" role="radiogroup" aria-label="Album cover style">
        {modes.map(option => (
          <button
            ...
            onClick={() => setCoverRenderingMode(option.id)}
          ...
          >
            <span className="flex items-center gap-2 font-semibold">
              ...
              {option.name}
              {mode === option.id && <Check className="ml-auto size-4 text-accent" aria-hidden="true" />}
            </span>
      ...
      <p className="text-sm leading-relaxed text-muted">
        This only sets the default for automatic covers. A cover you picked yourself for an album or folder always stays
        that photo, whichever style you choose. To go back to an automatic cover, use{' '}
        <span className="text-ink">Reset album cover</span> in that folder’s actions.
      </p>
    </SettingsSection>
  )
}
