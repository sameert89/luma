...
const media = (id: number, status: string, preview = status) =>
  ({ id, mediaType: 'image', thumbnail: { status }, preview: { status: preview } }) as Media

function Probe({ items, previews = false }: { items: Media[]; previews?: boolean }) {
  usePreviewPriority(items, previews)
  return null
}
