...
import { ChevronLeft, ChevronRight, Clapperboard, Heart, Info, Pause, Presentation, Settings2, Tag, HeartCrack, X } from 'lucide-react'
...
function storedSlideSeconds() {
  try { const value = Number(localStorage.getItem(slideSecondsKey)); return slideChoices.includes(value) ? value : 5 }
  catch { return 5 }
}
// Viewer only: Reels has its own auto-scroll.
const autoplayKey = 'luma-video-autoplay'
function storedAutoplay() {
  try { return localStorage.getItem(autoplayKey) === '1' }
  catch { return false }
}

export function Viewer({ active, filters, startSlideshow = false, onChange, onClose, onWatchReels, restoreFocus }: { active: Media; filters: Filters; startSlideshow?: boolean; onChange: (item: Media) => void; onClose: () => void; onWatchReels?: (item: Media) => void; restoreFocus: () => void }) {
  ...
  const navigate = useCallback((media: Media, direction: 'next' | 'previous') => { setDirection(direction); setContinued(false); onChange(media) }, [onChange])
  ...
  useEffect(() => { try { localStorage.setItem(autoplayKey, autoplayNext ? '1' : '0') } catch { /* the choice still applies to this visit */ } }, [autoplayNext])
  function videoEnded() {
    if (slideshow) { if (next) navigate(next, 'next'); else setSlideshow(false) }
    else if (autoplayNext && next?.mediaType === 'video') { navigate(next, 'next'); setContinued(true) }
  }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { if (next) navigate(next, 'next'); else setSlideshow(false) }, slideSeconds * 1000)
  ...
  useEffect(() => {
    function key(event: KeyboardEvent) {
      ...
      if (event.key.toLowerCase() === 's') { event.preventDefault(); setSlideshow(value => !value) }
    }
  ...
  return <>
    <Modal open={open} onOpenChange={setOpen} onClosed={onClose} title={item.fileName} description="Media viewer. Use Left and Right arrows to navigate, I for details, and Escape to close." wide hideHeader restoreFocus={restoreFocus}>
      <section className="relative flex min-h-0 flex-1 items-center justify-center bg-black" aria-label="Media preview">
        <div className="flex h-full w-full items-center justify-center overflow-hidden"><MediaStage item={item} direction={direction} autoPlay={slideshow || continued} autoplayNext={autoplayNext} onAutoplayNextChange={setAutoplayNext} optionsOpen={optionsOpen} onOptionsOpenChange={setOptionsOpen} onEnded={videoEnded} onNavigate={direction => { const target = direction === 'next' ? next : previous; if (target) navigate(target, direction) }} /></div>
        <header className={`absolute inset-x-0 top-0 flex items-center justify-between p-3 sm:p-5 ${chrome}`} data-chrome>
          ...
          <div className="flex shrink-0 items-center gap-2">
            {slideshow && <QuietButton className="border-transparent bg-canvas/85 tabular-nums" aria-label={`Slide duration, ${slideSeconds} seconds`} onClick={() => setSlideSeconds(value => slideChoices[(slideChoices.indexOf(value) + 1) % slideChoices.length])}>{slideSeconds} s</QuietButton>}
            <IconButton label="Slideshow" aria-pressed={slideshow} className="border-transparent bg-canvas/85" onClick={() => setSlideshow(value => !value)}>{slideshow ? <Pause className="size-5" /> : <Presentation className="size-5" />}</IconButton>
        ...
        <MediaActions preference={item.preference} onPreference={prefer} faded={chromeHidden}
          menuOpen={menuOpen} onMenuOpenChange={setMenuOpen} menuLabel="Viewer menu" className="bottom-24 phone-portrait:bottom-32">
          <IconButton label="View options" className={actionButton} onClick={() => setOptionsOpen(true)}><Settings2 className="size-4" /></IconButton>
          <IconButton label="Media details" className={actionButton} onClick={() => setDetailsOpen(true)}><Info className="size-4" /></IconButton>
          {motion && onWatchReels && <IconButton label="Watch on Reels" className={actionButton} onClick={() => onWatchReels(item)}><Clapperboard className="size-4" /></IconButton>}
          <IconButton label="Previous item" className={actionButton} disabled={!previous} onClick={() => previous && navigate(previous, 'previous')}><ChevronLeft className="size-4" /></IconButton>
          <IconButton label="Next item" className={actionButton} disabled={!next} onClick={() => next && navigate(next, 'next')}><ChevronRight className="size-4" /></IconButton>
        </MediaActions>
        {neighbors.isError && <p role="alert" className="absolute bottom-24 rounded-full bg-canvas px-4 py-2 text-sm text-danger">{errorMessage(neighbors.error)}</p>}
      </section>
    </Modal>
    <Modal open={detailsOpen} onOpenChange={setDetailsOpen} title="Media details" description="Tags and file information." sheet>
      <div className="space-y-6 overflow-auto p-5"><div className="flex gap-2"><QuietButton aria-pressed={item.preference === 'liked'} onClick={() => prefer(item.preference === 'liked' ? 'neutral' : 'liked')}><Heart className={`size-4 ${item.preference === 'liked' ? 'fill-accent text-accent' : ''}`} />Like</QuietButton><QuietButton aria-pressed={item.preference === 'disliked'} onClick={() => prefer(item.preference === 'disliked' ? 'neutral' : 'disliked')}><HeartCrack className={`size-4 ${item.preference === 'disliked' ? 'text-danger' : ''}`} />Dislike</QuietButton></div>
