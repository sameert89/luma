...
export function Viewer({ active, filters, startSlideshow = false, onChange, onClose, onWatchReels, restoreFocus }: { active: Media; filters: Filters; startSlideshow?: boolean; onChange: (item: Media) => void; onClose: () => void; onWatchReels?: (item: Media) => void; restoreFocus: () => void }) {
  ...
  const preference = useMutation({ mutationFn: (value: 'liked' | 'disliked' | 'neutral') => request(`/api/media/${active.id}/preference`, undefined, 'PUT', { preference: value }), onSuccess: async () => {
  ...
  return <>
    <Modal open={open} onOpenChange={setOpen} onClosed={onClose} title={item.fileName} description="Media viewer. Use Left and Right arrows to navigate, I for details, and Escape to close." wide hideHeader restoreFocus={restoreFocus}>
      <section className="relative flex min-h-0 flex-1 items-center justify-center bg-black" aria-label="Media preview">
        ...
        {/* Same row, same height for photos and videos: just above the video player bar. */}
        <MediaActions preference={item.preference} pending={preference.isPending} onPreference={value => preference.mutate(value)} faded={chromeHidden}
          menuOpen={menuOpen} onMenuOpenChange={setMenuOpen} menuLabel="Viewer menu" className="bottom-20 phone-portrait:bottom-24">
          ...
          <IconButton label="Previous item" className={actionButton} disabled={!previous || preference.isPending} onClick={() => previous && navigate(previous, 'previous')}><ChevronLeft className="size-4" /></IconButton>
          <IconButton label="Next item" className={actionButton} disabled={!next || preference.isPending} onClick={() => next && navigate(next, 'next')}><ChevronRight className="size-4" /></IconButton>
        </MediaActions>
