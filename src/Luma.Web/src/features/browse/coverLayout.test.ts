...
describe('album cover layout', () => {
  ...
  it('builds a 3–5 tile mosaic for three or more automatic images, sized to the card', () => {
    ...
    expect(coverLayout({ images: five, override: false, mode: 'smart', containerAspect: wideCard })).toMatchObject({ kind: 'mosaic', tiles: { length: 5 } })
    expect(coverLayout({ images: five, override: false, mode: 'smart', containerAspect: 2 })).toMatchObject({ kind: 'mosaic', tiles: { length: 4 } })
    expect(coverLayout({ images: five, override: false, mode: 'smart', containerAspect: phoneCard })).toMatchObject({ kind: 'mosaic', tiles: { length: 3 } })
    expect(coverLayout({ images: five.slice(0, 3), override: false, mode: 'smart', containerAspect: wideCard })).toMatchObject({ kind: 'mosaic', tiles: { length: 3 } })
    // A narrow phone card keeps three legible tiles even when its shape could fit more.
    expect(coverLayout({ images: five, override: false, mode: 'smart', containerAspect: 1.8, containerWidth: 172 })).toMatchObject({ kind: 'mosaic', tiles: { length: 3 } })
  })
  ...
  it('leads the mosaic with the image that suits the large tile best', () => {
    ...
    const layout = coverLayout({ images: [portrait(1), landscape(2), square], override: false, mode: 'smart', containerAspect: phoneCard })
  ...
  it('adapts a single image to the card: cover, mild crop, or enlarged over a blurred backdrop', () => {
    expect(coverLayout({ images: [landscape(1)], override: false, mode: 'smart', containerAspect: 1.6 })).toMatchObject({ fit: 'cover' })
    expect(coverLayout({ images: [landscape(1)], override: false, mode: 'smart', containerAspect: wideCard })).toMatchObject({ fit: 'smart-cover', scale: 1.4 })
    // A portrait photo in a wide desktop banner: mismatch 5.7, never a postage stamp.
    expect(coverLayout({ images: [portrait(1), portrait(2)], override: false, mode: 'smart', containerAspect: wideCard })).toMatchObject({ fit: 'blurred-foreground', scale: 1.25 })
  })
  ...
  it('always fills the card in cropped mode, for automatic and manual covers alike', () => {
    for (const override of [false, true])
      expect(coverLayout({ images: [portrait(1), landscape(2), landscape(3)], override, mode: 'cropped', containerAspect: wideCard }))
  ...
  it('has no cover without usable images', () => {
    ...
    expect(coverLayout({ images: [{ url: '/x', width: 0, height: 0 }], override: false, mode: 'cropped', containerAspect: wideCard })).toBeNull()
  })
