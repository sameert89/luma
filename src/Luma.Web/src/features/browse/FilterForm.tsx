...
export function FilterForm({ value, onApply }: { value: Filters; onApply: (filters: Filters) => void }) {
  ...
  return <form className="flex min-h-0 min-w-0 flex-col" onSubmit={event => { event.preventDefault(); onApply(draft) }}><div className="min-w-0 space-y-6 p-5">
    ...
    <FilterSection title="Location and name"><div className={twoColumns}><Field label="Library"><Select value={draft.libraryId ?? ''} onChange={event => setDraft(old => ({ ...old, libraryId: event.target.value ? Number(event.target.value) : undefined, folderId: undefined }))}><option value="">All libraries</option>{libraries.data?.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></Field>{text('q', 'Keyword')}{text('path', 'Path contains')}{text('startsWith', 'Name starts with')}{text('endsWith', 'Name ends with')}<Field label="Extensions"><Input value={draft.extension?.join(', ') ?? ''} placeholder="jpg, png, mp4" onChange={event => setDraft(old => ({ ...old, extension: event.target.value.split(',').map(x => x.trim()).filter(Boolean) }))} /></Field></div>
    ...
    <FilterSection title="Tags"><Field label="Match tags"><Select value={draft.tagMode ?? 'all'} onChange={event => set('tagMode', event.target.value)}><option value="all">All selected tags</option><option value="any">Any selected tag</option></Select></Field><Field label="Find a tag"><Input value={tag} onChange={event => setTag(event.target.value)} placeholder="Search existing tags" /></Field>{tag && <ul className="mt-3 flex flex-wrap gap-2" aria-label="Matching tags">{suggestions.data?.map(item => <li key={item.id}><QuietButton className="min-h-9" onClick={() => { setDraft(old => ({ ...old, tagged: undefined, tag: [...new Set([...(old.tag ?? []), item.name])] })); setTag('') }}>{item.name}</QuietButton></li>)}</ul>}<div className="flex flex-wrap gap-2">{draft.tag?.map(name => <QuietButton key={name} className="min-h-9" aria-label={`Remove filter tag ${name}`} onClick={() => setDraft(old => ({ ...old, tag: old.tag?.filter(x => x !== name) }))}>{name} ×</QuietButton>)}</div></FilterSection>
