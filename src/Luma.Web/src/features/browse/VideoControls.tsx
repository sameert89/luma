...
const clockEvents = ['timeupdate', 'durationchange', 'loadedmetadata', 'progress', 'seeking', 'seeked', 'emptied'] as const
...
export function usePlaybackClock(video: RefObject<HTMLVideoElement | null>, mediaId: number): Clock {
  ...
  useEffect(() => {
    ...
    function read() {
      ...
      setClock(old => old.time === time && old.duration === duration && old.loaded === loaded ? old : { time, duration, loaded })
    }
    function tick() { read(); frame = requestAnimationFrame(tick) }
    function start() { cancelAnimationFrame(frame); frame = requestAnimationFrame(tick) }
    function stop() { cancelAnimationFrame(frame); read() }
...
function useScrub(video: RefObject<HTMLVideoElement | null>) {
  ...
  function change(position: number) {
    if (!dragging.current) { seek(position, true); return }
  ...
  function begin() { dragging.current = true }
  function end() {
...
function percent(value: number, duration: number) { return duration > 0 ? Math.min(100, Math.max(0, value / duration * 100)) : 0 }

function Scrubber({ video, mediaId, label, className = '', interactive = true, onInteract, onFocus, onBlur, children }: {
  video: RefObject<HTMLVideoElement | null>; mediaId: number; label: string; className?: string; interactive?: boolean
  onInteract?: () => void; onFocus?: () => void; onBlur?: () => void; children?: (clock: Clock, shown: number) => ReactNode
}) {
  const clock = usePlaybackClock(video, mediaId)
  const scrub = useScrub(video)
  const shown = scrub.value ?? clock.time
  return <>
    <SeekRange aria-label={label} aria-valuetext={`${formatTime(shown)} of ${formatTime(clock.duration)}`} min={0} max={clock.duration} step={0.1} value={shown}
      played={percent(shown, clock.duration)} loaded={percent(clock.loaded, clock.duration)} disabled={clock.duration <= 0} className={className}
      onPointerDown={() => { if (interactive) scrub.begin() }} onPointerUp={scrub.end} onPointerCancel={scrub.end}
      onKeyUp={scrub.end} onFocus={onFocus} onBlur={() => { scrub.end(); onBlur?.() }}
      onChange={event => { onInteract?.(); if (interactive) scrub.change(Number(event.target.value)) }} />
...
export function ReelsSeek({ video, mediaId, visible, stripRef, onReveal }: {
