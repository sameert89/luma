...
export function SourceVerificationSettings() {
  ...
  return <SettingsSection title="Missing-file checks" description="Choose whether Luma regularly checks indexed files on disk between scans.">
    ...
    {setting.data && <label className="flex max-w-2xl items-start gap-3 text-sm">
      <Checkbox aria-label="Check indexed files for deletions" checked={setting.data.enabled} disabled={update.isPending} onChange={event => update.mutate(event.target.checked)} />
