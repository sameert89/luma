...
function savedReelsState(query: string) {
  try {
    ...
    return {
      ...
      mediaId:
        saved?.query === query && Number.isSafeInteger(saved?.mediaId) && saved.mediaId > 0
          ? (saved.mediaId as number)
          : null,
    }
  } catch {
    return { muted: true, autoScroll: false, mediaId: null }
  }
}
...
export function Reels({
  ...
  const first = useQuery({
    queryKey: ['reels-first', queryFilters],
    queryFn: ({ signal }) => request<MediaPage>(`/api/media?${queryString({ ...queryFilters, limit: 1 })}`, signal),
    enabled: !restoredId,
    gcTime: 0,
  })
  ...
  const detail = useQuery({
    queryKey: ['detail', baseId],
    queryFn: ({ signal }) => request<Media>(`/api/media/${baseId}`, signal),
    enabled: !!baseId,
    gcTime: 0,
  ...
  useEffect(() => {
    if (
      restoredId &&
      !active &&
      (detail.data?.availability === 'missing' || (detail.error instanceof ApiError && detail.error.status === 404))
    )
      setRestoredId(null)
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        reelsStateKey,
        JSON.stringify({ muted, autoScroll, query: positionKey, mediaId: item?.id ?? restoredId }),
      )
    } catch {
      /* Storage may be unavailable; current controls still work. */
    }
  }, [muted, autoScroll, positionKey, item?.id, restoredId])
  const neighbors = useQuery({
    queryKey: ['reels-neighbors', queryFilters, item?.id],
    queryFn: ({ signal }) =>
      request<Neighbors>(`/api/media/${item!.id}/neighbors?${queryString(queryFilters)}`, signal),
    enabled: !!item,
    gcTime: 0,
  })
  ...
  function navigate(direction: 'next' | 'previous') {
    const target = neighbors.data?.[direction]
    if (target) {
  ...
  useEffect(() => {
    function key(event: KeyboardEvent) {
      if (
        ...
        (event.target instanceof HTMLElement && event.target.closest('input,select,textarea,video,button'))
      )
        return
      if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
        event.preventDefault()
        const move = event.key === 'ArrowDown' ? 'next' : 'previous'
        const target = neighbors.data?.[move]
        if (target) {
          setDirection(move)
          setActive(target)
        }
  ...
  if (!item && detail.isError && !(detail.error instanceof ApiError && detail.error.status === 404))
    return (
      <p role="alert" className="p-5 text-danger">
  ...
  if (!item) {
    ...
    return (
      <section
        aria-labelledby="reels-state-title"
        className="flex min-h-0 flex-1 items-center justify-center bg-black px-6 text-center text-white"
      >
        <div role={loading ? 'status' : undefined} className="flex max-w-sm flex-col items-center gap-4">
          {loading ? (
            <LoaderCircle className="size-10 text-white/60 motion-safe:animate-spin" aria-hidden="true" />
          ) : (
            <span className="flex size-16 items-center justify-center rounded-full border border-white/15 bg-white/10">
              <Clapperboard className="size-7 text-white/70" aria-hidden="true" />
            </span>
          )}
          <h1 id="reels-state-title" className="text-xl font-semibold">
            {loading ? 'Loading reels…' : 'No reels to show'}
          </h1>
          {!loading && (
            <>
              <p className="text-sm leading-relaxed text-white/60">
                No photos or videos match this view. Adjust the filters or choose another folder.
              </p>
              <QuietButton className="border-white/20 text-white hover:bg-white/10" onClick={onFilters}>
  ...
  return (
    <section ref={root} aria-label="Reels" className="relative flex min-h-0 flex-1 flex-col bg-black">
      ...
      <div className="flex min-h-0 flex-1">
        <MediaStage
          ...
          suspended={suspended}
          loop={!(autoScroll && next)}
          fullscreenRoot={root}
          onOpenViewer={onOpenViewer}
          onNavigate={navigate}
          onLike={() => {
            if (!liked) preference.mutate({ id: item.id, value: 'liked' })
          }}
          onDislike={() => {
            if (item.preference !== 'disliked') preference.mutate({ id: item.id, value: 'disliked' })
          }}
          onEnded={() => {
            if (!suspended && autoScroll && next) {
              setDirection('next')
              setActive(next)
            }
          }}
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex items-start p-3 sm:p-5">
        <p className="pointer-events-auto rounded-full bg-canvas/80 px-3 py-1 text-lg font-semibold tracking-tight">
          luma<span className="text-accent">.</span>
          <Clapperboard className="ml-1 inline size-3 align-super text-accent" aria-hidden="true" />
      ...
      <div hidden={isFullscreen}>
        <MediaActions
          ...
          onPreference={value => preference.mutate({ id: item.id, value })}
          menuOpen={menuOpen}
          onMenuOpenChange={setMenuOpen}
          menuLabel="Reels menu"
          className="bottom-16"
        >
          <IconButton label="Filters" className={actionButton} onClick={onFilters}>
            <SlidersHorizontal className="size-4" />
          </IconButton>
          {onToggleFilters && (
            <IconButton
            ...
            >
              {filtersVisible ? <EyeOff className="size-4" /> : <Eye className="size-4" />}
            </IconButton>
          )}
          <IconButton
          ...
          >
            <Timer className={`size-4 ${autoScroll ? 'text-accent' : ''}`} />
          </IconButton>
          <IconButton label="Tags" className={actionButton} aria-pressed={tags} onClick={() => setTags(!tags)}>
            <Tag className="size-4" />
          </IconButton>
          <IconButton label="View options" className={actionButton} onClick={() => setOptionsOpen(true)}>
            <Settings2 className="size-4" />
          </IconButton>
          <IconButton label="Media information" className={actionButton} onClick={() => setInfoOpen(true)}>
          ...
          <IconButton label="Next" className={actionButton} disabled={!next} onClick={() => navigate('next')}>
            <ChevronDown className="size-4" />
          </IconButton>
        </MediaActions>
      </div>
      <Modal open={tags} onOpenChange={setTags} title="Tags" description="Add or remove tags for this item." sheet>
      ...
      <Modal
        ...
        description="File information for this reel."
        sheet
      >
        <div className="overflow-auto p-5">
          <MediaInformation item={item} />
          <div className="mt-6">
            <MetadataExchange mediaIds={[item.id]} />
          </div>
        </div>
      </Modal>
      {preference.isError && (
        <p role="alert" className="absolute bottom-28 z-10 rounded-full bg-canvas px-4 py-2 text-sm text-danger">
          {errorMessage(preference.error)}
        </p>
      )}
      {neighbors.isError && (
        <p role="alert" className="absolute bottom-20 z-10 rounded-full bg-canvas px-4 py-2 text-sm text-danger">
