...
it('waits for the root folder listing instead of announcing an empty library', async () => {
  vi.stubGlobal(
    'fetch',
    vi.fn().mockResolvedValue(new Response(JSON.stringify({ items: [], nextCursor: null, previousCursor: null }))),
  )
  const client = new QueryClient({ defaultOptions: { queries: { retry: false, gcTime: 0 } } })
  const props = {
    filters: { libraryId: 1 },
  ...
  rerender(
    <QueryClientProvider client={client}>
      <Gallery {...props} leadingContent={<p>Subfolders</p>} />
    </QueryClientProvider>,
  )
  expect(screen.queryByText('No media to show')).not.toBeInTheDocument()
...
it('keeps accepted folder indexing in a loading state while its scan status resolves', async () => {
    ...
    vi.fn((url: string) =>
      url.startsWith('/api/scans/')
        ...
        : Promise.resolve(
            new Response(
              JSON.stringify(
                url.endsWith('/index') ? { id: 1 } : { items: [], nextCursor: null, previousCursor: null },
              ),
            ),
          ),
    ),
  ...
  await act(async () => {
    finish(
      new Response(JSON.stringify({ id: 1, state: 'completed', discovered: 0, ready: 0, pending: 0, processing: 0 })),
    )
  })
  expect(await screen.findByText('No media to show')).toBeVisible()
})
...
it('renders a heading per tag group with its media below it, repeating an item under each of its tags', async () => {
  const item = (id: number, fileName: string, groupKey: string, groupLabel: string) => ({
    ...
    thumbnail: { status: 'ready', url: `/thumb/${id}` },
    preview: { status: 'ready', url: `/preview/${id}` },
    tags: [],
    groupKey,
    groupLabel,
  })
  const items = [
    item(1, 'one.jpg', 'tag:4', 'Beach'),
    item(2, 'two.jpg', 'tag:4', 'Beach'),
    item(2, 'two.jpg', 'tag:9', 'Sunset'),
    item(3, 'three.jpg', 'tag:9', 'Sunset'),
  ]
  vi.stubGlobal(
    'fetch',
    vi.fn().mockResolvedValue(new Response(JSON.stringify({ items, nextCursor: null, previousCursor: null }))),
  ...
  Object.defineProperty(HTMLElement.prototype, 'offsetHeight', {
    ...
    get(this: HTMLElement) {
      return this.dataset.testid === 'gallery-scroll' ? 600 : 0
    },
  })
  try {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false, gcTime: 0 } } })
...
it('refreshes the collection when the gallery is pulled down from the top', async () => {
  vi.stubGlobal(
    ...
    vi
      .fn()
      .mockImplementation(() =>
        Promise.resolve(new Response(JSON.stringify({ items: [], nextCursor: null, previousCursor: null }))),
      ),
  )
