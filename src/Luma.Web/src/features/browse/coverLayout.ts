...
export type CoverLayout =
  ...
  { kind: 'single'; image: CoverImage; fit: SingleFit; scale: number } | { kind: 'mosaic'; tiles: CoverImage[] }

const storageKey = 'luma-album-covers'
const listeners = new Set<() => void>()
let current: CoverRenderingMode = (() => {
  try {
    return localStorage.getItem(storageKey) === 'cropped' ? 'cropped' : 'smart'
  } catch {
    return 'smart'
  }
})()
...
export const mismatchOf = (imageAspect: number, containerAspect: number) =>
  Math.max(imageAspect / containerAspect, containerAspect / imageAspect)
...
export function coverLayout({
  ...
  if (mode === 'smart' && !override && suitable.length >= 3) {
    ...
    const lead = tiles.reduce((best, image) =>
      mismatchOf(aspect(image), hero) < mismatchOf(aspect(best), hero) ? image : best,
    )
  ...
  const scale =
    fit === 'cover' ? mismatch : Math.min(mismatch, fit === 'smart-cover' ? smartCropScale : foregroundScale)
