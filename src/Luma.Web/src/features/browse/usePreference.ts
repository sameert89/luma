...
export function usePreference() {
  ...
  return useMutation({
    ...
    mutationFn: ({ id, value }: { id: number; value: Preference }) =>
      request(`/api/media/${id}/preference`, undefined, 'PUT', { preference: value }),
    ...
    onError: (_error, { id }, context) => {
      if (context?.previous) client.setQueryData(['detail', id], context.previous)
    },
