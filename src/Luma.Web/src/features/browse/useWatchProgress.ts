...
export function useWatchProgress(item: Media, video: RefObject<HTMLVideoElement | null>, reels: boolean) {
  ...
  const progress = useQuery({
    queryKey: ['watch-progress', item.id],
    queryFn: ({ signal }) => request<Progress>(`/api/media/${item.id}/progress`, signal),
    enabled: item.mediaType === 'video',
    initialData: item.watchProgress ?? undefined,
    gcTime: 0,
  })
  latest.current = progress.data ?? null
  const resume =
    !reels && dismissed !== item.id && progress.data?.state === 'in_progress' && progress.data.positionSeconds > 5
  ...
  useEffect(() => {
    ...
    function accrue() {
      ...
      if (Number.isFinite(player!.duration) && player!.duration > 0)
        snapshot = {
          positionSeconds: Math.min(player!.currentTime, player!.duration),
    ...
    function save() {
      ...
      const delta = Math.min(30, watched)
      watched -= delta
      const body = { ...snapshot, watchedSeconds: delta }
      saving = true
      void (async () => {
        const response = await fetch(`/api/media/${item.id}/progress`, {
          method: 'PUT',
          keepalive: true,
        ...
        if (client.getQueryCache().find({ queryKey: ['watch-progress', item.id] }))
          client.setQueryData(['watch-progress', item.id], value)
    ...
    return () => {
      ...
      player.removeEventListener('waiting', stop)
  ...
  function choose(fromBeginning: boolean) {
    const player = video.current
    if (player) {
      player.currentTime = fromBeginning ? 0 : (latest.current?.positionSeconds ?? 0)
