...
const image = {
  ...
  width: 640,
  height: 480,
  preview: { url: '/cached.jpg', status: 'ready' },
} as Media
...
describe('media viewing', () => {
  it('shows the original over its thumbnail until a preview exists and does not swap mid-visit', () => {
    const pending = {
      ...image,
      extension: '.jpg',
      thumbnail: { url: '/thumb.webp', status: 'ready' },
      preview: { url: '/cached.jpg', status: 'pending' },
    } as Media
    ...
    rerender(
      <MediaStage item={{ ...pending, preview: { ...pending.preview, status: 'ready' } }} onNavigate={vi.fn()} />,
    )
    ...
    rerender(
      <MediaStage
        item={{ ...pending, id: 2, preview: { ...pending.preview, status: 'ready' } }}
        onNavigate={vi.fn()}
      />,
    )
  ...
  it('keeps options outside the slide and resets photo transforms and original access across repeated navigation', async () => {
    ...
    for (const id of [2, 3, 4, 5]) {
      rerender(
        <MediaStage
          item={{ ...image, id, fileName: `${id}.jpg`, preview: { ...image.preview, url: `/cached-${id}.jpg` } }}
          onNavigate={vi.fn()}
        />,
      )
  ...
  it('uses cached images until explicit original access and supports keyboard zoom and rotation', async () => {
    ...
    expect(screen.getByRole('link', { name: 'Download' })).toHaveAttribute(
      'href',
      '/api/media/1/original?download=true',
    )
  ...
  it('retains playback after StrictMode setup, handles blocked autoplay, and releases inactive playback', async () => {
    ...
    try {
      const { container, unmount } = render(
        <StrictMode>
          <MediaStage item={{ ...image, mediaType: 'video' }} reels muted onNavigate={vi.fn()} />
        </StrictMode>,
      )
  ...
  it('pauses reels on a centre tap, likes on a double tap and reveals seeking from the bottom strip', async () => {
    ...
    try {
      const { container } = render(
        <MediaStage item={{ ...image, mediaType: 'video' }} reels muted onNavigate={vi.fn()} onLike={onLike} />,
      )
  ...
  it('skips on side double taps, plays at 2× while held and shows buffering only for real stalls', async () => {
    ...
    try {
      ...
      act(() => {
        stage.dispatchEvent(new PointerEvent('pointerdown', hold))
      })
      await act(async () => {
      ...
      act(() => {
        stage.dispatchEvent(new PointerEvent('pointerup', hold))
      })
  ...
  it('auto-hides fullscreen controls during playback and always offers a visible exit', async () => {
    ...
    try {
      const { container, rerender } = render(
        <MediaStage item={{ ...image, mediaType: 'video' }} onNavigate={vi.fn()} />,
      )
      const video = container.querySelector('video')!
  ...
  it('animates GIFs from the original and loops reels when asked', () => {
    ...
    try {
      const { unmount } = render(
        <MediaStage item={{ ...image, fileName: 'loop.gif', extension: '.gif' }} onNavigate={vi.fn()} />,
      )
      expect(screen.getByRole('img')).toHaveAttribute('src', '/api/media/1/original')
      unmount()
...
it('offers persisted resume and start-over choices only for a partially watched normal video', async () => {
  ...
  const { container, rerender } = render(
    <MediaStage item={{ ...image, mediaType: 'video', watchProgress: progress }} onNavigate={vi.fn()} />,
  )
  ...
  rerender(
    <MediaStage
      item={{ ...image, id: 2, mediaType: 'video', watchProgress: { ...progress, mediaId: 2 } }}
      onNavigate={vi.fn()}
    />,
  )
