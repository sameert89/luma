...
export function GalleryActions({ folder, libraryName, ancestors, onHidden, onSlideshow, onRefresh, filtersVisible, onToggleFilters, onFilters, onClearFilters }: {
  ...
  if (folder) return <FolderActions folder={folder} libraryName={libraryName} ancestors={ancestors} onHidden={onHidden} extraActions={actions} />
  return <><IconButton label="Gallery actions" onClick={() => setOpen(true)}><EllipsisVertical className="size-4" /></IconButton><Modal open={open} onOpenChange={setOpen} title="Gallery actions" description="Slideshow, selection and filters." sheet><div className="flex flex-wrap gap-2 p-5">{actions(() => setOpen(false))}</div></Modal></>
}
