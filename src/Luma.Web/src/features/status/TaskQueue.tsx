...
const kindLabels: Record<string, string> = {
  ...
  import: 'Tag import',
  xmp: 'XMP export',
  dislikes: 'Disliked paths export',
}
...
const refresh = (client: QueryClient) =>
  Promise.all(
    ['tasks', 'indexing', 'scan', 'media', 'folders', 'libraries', 'metadata-job'].map(key =>
...
export function TaskQueue() {
  ...
  const tasks = useQuery({
    queryKey: ['tasks'],
    queryFn: ({ signal }) => request<Task[]>('/api/tasks', signal),
    refetchInterval: query => (starting || query.state.data?.some(isActive) ? 2000 : 10000),
  })
  const clear = useMutation({
    mutationFn: () => request('/api/tasks/clear-finished', undefined, 'POST'),
  ...
  return (
    <section aria-label="Task queue" className="space-y-3">
      <div className="flex items-center justify-between gap-3 py-2">
        <p className="text-sm font-semibold">
          Jobs{running > 0 && <span className="font-normal text-muted"> · {running} active</span>}
        </p>
        <QuietButton disabled={!finished || clear.isPending} onClick={() => clear.mutate()}>
          {clear.isPending ? 'Clearing…' : 'Clear finished'}
        </QuietButton>
      </div>
      {indexing.data?.cachePressure && (
        <p role="status" className="text-sm text-danger">
          Preview storage is full, so preview preparation is paused.
        </p>
      )}
      {clear.isError && (
...
function TaskRow({ task }: { task: Task }) {
  ...
  const action = useMutation({
    mutationKey: ['task-action', task.id],
    mutationFn: (operation: 'cancel' | 'queue' | 'clear') =>
      request(`/api/tasks/${task.id}/${operation}`, undefined, 'POST'),
  ...
  return (
    <article
      aria-label={`${name}, ${stateLabels[task.state] ?? task.state}`}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {task.scope && (
            <p className="truncate text-sm text-muted" title={task.scope}>
              {task.scope}
            </p>
          )}
        </div>
        <p className={`shrink-0 text-sm font-medium ${stateTones[task.state] ?? 'text-muted'}`}>
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {isActive(task) ? (
          confirming ? (
            <div className="space-y-2">
              <p className="text-sm leading-relaxed">
                Files found so far stay in your library, and their previews are made as you view them. Each folder you
                open is finished then, including its tags if the library imports tags.
              </p>
              <div className="flex flex-wrap gap-2">
                <QuietButton disabled={busy} onClick={() => action.mutate('cancel')}>
                  {busy ? 'Stopping…' : 'Stop indexing'}
                </QuietButton>
                <QuietButton disabled={busy} onClick={() => setConfirming(false)}>
                  Keep running
                </QuietButton>
              </div>
            </div>
            ...
            <QuietButton
              ...
              disabled={busy}
              onClick={() => (task.kind === 'indexing' ? setConfirming(true) : action.mutate('cancel'))}
            >
          ...
          <>
            {canQueueAgain(task) && (
              <QuietButton aria-label={`Queue ${name} again`} disabled={busy} onClick={() => action.mutate('queue')}>
                Queue again
              </QuietButton>
            )}
            <QuietButton aria-label={`Clear ${name}`} disabled={busy} onClick={() => action.mutate('clear')}>
              Clear
            </QuietButton>
          </>
        )}
