...
const task = (id: string, state: string, kind = 'import') => ({
  id,
  kind,
  state,
  createdAt: '2026',
...
const renderQueue = (children = <TaskQueue />) =>
  render(
...
it('shows an operation while enqueue is pending and shows its queued state immediately after acceptance', async () => {
  ...
  vi.stubGlobal(
    'fetch',
    vi.fn(() => Promise.resolve(new Response(JSON.stringify(accepted ? [task('metadata-1', 'queued')] : [])))),
  )
  function Operation() {
...
it('offers Clear on every finished task and Queue again only on stopped ones', async () => {
  let tasks = [
    task('scan-1', 'interrupted', 'indexing'),
    task('metadata-2', 'completed', 'xmp'),
    task('metadata-3', 'failed'),
    task('metadata-4', 'running'),
  ]
  const fetcher = vi.fn((url: string) => {
    if (url === '/api/tasks/scan-1/clear') {
      tasks = tasks.filter(item => item.id !== 'scan-1')
...
it('clears every finished entry, retains active work and disables clearing when none remain', async () => {
  let tasks = ['queued', 'running', 'completed', 'cancelled', 'failed', 'interrupted', 'expired'].map((state, index) =>
    task(`metadata-${index + 1}`, state),
  )
  const fetcher = vi.fn((url: string) => {
    if (url === '/api/tasks/clear-finished') {
      tasks = tasks.filter(item => ['queued', 'running'].includes(item.state))
      return Promise.resolve(new Response(null, { status: 204 }))
    }
    return Promise.resolve(new Response(JSON.stringify(tasks)))
  })
...
it('explains what stopping indexing means before cancelling it', async () => {
  ...
  const fetcher = vi.fn((url: string) => {
    ...
    return Promise.resolve(
      new Response(JSON.stringify(url === '/api/tasks' ? [task('scan-1', state, 'indexing')] : {})),
    )
  })
  vi.stubGlobal('fetch', fetcher)
  renderQueue()
