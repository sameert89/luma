...
function useScanRefresh(libraryId?: number) {
  ...
  const status = useQuery({ queryKey: ['indexing'], queryFn: ({ signal }) => request<IndexingStatus>('/api/indexing', signal), refetchInterval: 5000, enabled: !!libraryId })
  const id = status.data?.libraries.find(item => item.id === libraryId)?.latestScanId
  const scan = useQuery({ queryKey: ['scan', id], queryFn: ({ signal }) => request<Scan>(`/api/scans/${id}`, signal), enabled: !!id, gcTime: 0,
    refetchInterval: query => { const data = query.state.data; return !data || data.state === 'queued' || data.state === 'running' || data.pending + data.processing > 0 ? 3000 : false } }).data
  useEffect(() => {
    if (!scan) return
