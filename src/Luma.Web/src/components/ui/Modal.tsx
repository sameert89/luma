...
export function Modal({
  ...
  useEffect(() => {
    ...
    let active = true
    function back() {
      if (window.history.state?.lumaModal !== historyMarker.current) change.current(false)
    }
    function attach() {
      if (!active) return
      if (!historyMarker.current || window.history.state?.lumaModal !== historyMarker.current) {
        ...
        window.history.pushState(
          { ...window.history.state, lumaModal: historyMarker.current },
          '',
      ...
      <Primitive.Portal>
        {open && (
          <>
            ...
            <Primitive.Content
              data-motion={wide ? 'viewer' : sheet ? 'sheet' : 'dialog'}
              tabIndex={-1}
              ...
              onPointerDown={event => {
                if (!wide && event.target === event.currentTarget) {
                  swallowNextClick()
                  onOpenChange(false)
                }
              }}
              className={
                wide
                  ...
                  : 'fixed inset-0 z-50 flex h-dvh w-full items-end justify-center sm:items-center sm:p-4'
              }
            >
              {wide ? (
                <>
                  ...
                  {!hideHeader && (
                    <header className="flex shrink-0 items-center justify-between gap-4 border-b border-line px-5 py-3">
                      <div className="min-w-0">
                        <Primitive.Title className="truncate text-lg font-semibold">{title}</Primitive.Title>
                        <Primitive.Description className="sr-only">{description}</Primitive.Description>
                      </div>
                ...
                <div
                  data-motion-panel="true"
                  className="sheet-panel flex w-full min-h-0 flex-col overflow-hidden rounded-t-3xl border border-line bg-canvas sm:max-w-xl sm:rounded-2xl"
                >
                  {!hideHeader && (
                    <header className="flex shrink-0 items-center justify-between gap-4 border-b border-line px-5 py-3">
                      <div className="min-w-0">
