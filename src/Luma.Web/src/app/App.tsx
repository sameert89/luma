...
import { Bookmark, Check, ChevronRight, CircleHelp, Clapperboard, CopyCheck, FolderOpen, GitFork, Images, ListChecks, LoaderCircle, Search, Settings, Tag, X } from 'lucide-react'
...
export function App() {
  ...
  useEffect(() => { if (unseenRelease()) setWhatsNewOpen(true) }, [])
  ...
  function navigateSection(target: Section) {
    ...
    else if (target === 'reels') apply(ungroupTags({ ...filters, recursive: filters.folderId ? true : filters.recursive, mediaType: filters.mediaType ?? reelsMediaType }), target)
  ...
  const pullRescan = useMutation({ mutationKey: ['background-task'],
    mutationFn: async () => { if (rescanFolderId) await request(`/api/folders/${rescanFolderId}/scans`, undefined, 'POST', { metadataMode: library?.metadataMode ?? 'embedded' }) },
    onSuccess: async () => {
      setRefreshError('')
      await Promise.all(['tasks', 'indexing', 'scan', 'media', 'folders', 'libraries'].map(key => client.invalidateQueries({ queryKey: [key] })))
  ...
  return <div className="flex h-dvh min-h-0 flex-col overflow-hidden bg-canvas text-ink" onScrollCapture={event => {
    ...
    {section !== 'reels' && <SearchHeader value={search} onChange={setSearch} onHome={() => apply({})} onFilters={() => setFilterOpen(true)} onSearch={submitSearch} onSuggestion={chooseSuggestion} onClear={clearSearch} searchRequested={searchFocusRequested} scope={`${section}:${queryString(filters)}`} />}
        ...
        {section === 'settings' ? <div className="overflow-auto" data-scroll-restore><UpdateSettings onWhatsNew={() => setWhatsNewOpen(true)} /><ThemePicker value={theme} onChange={setTheme} /><AlbumCoverSettings /><SettingsSection title="Random media URL" description="Generate a reusable random-media link using your current browsing filters."><RandomUrls filters={filters} /></SettingsSection><SettingsSection title="Metadata exchange" description="Download metadata and disliked paths from your library."><MetadataExchange dislikes heading={false} /></SettingsSection><LibrarySettings /><HiddenFolders /><SettingsSection title="Help" description="See what each button, gesture and keyboard shortcut does."><QuietLink data-help-entry href={`?${helpParams}`} className="w-fit" onClick={event => { event.preventDefault(); openHelp() }}><CircleHelp className="size-4" aria-hidden="true" />Open the Help guide</QuietLink></SettingsSection><SettingsSection title="About" description="Version and project information for this Luma installation."><div className="flex flex-col items-start gap-2 text-sm"><p><span className="font-medium">Luma</span> <span className="text-muted">v{packageJson.version}</span></p><p className="text-muted">Created by Sameer Trivedi.</p><QuietLink href="https://github.com/sameert89/luma" target="_blank" rel="noreferrer"><GitFork className="size-4" aria-hidden="true" />GitHub</QuietLink></div></SettingsSection></div> : section === 'help' ? <Help onBack={closeHelp} /> : <>
          {section !== 'reels' && <div className="shrink-0 space-y-2 px-4 pb-2 pt-3 sm:px-6 sm:pb-3 sm:pt-4"><div className="flex min-w-0 items-center gap-2">
            ...
            <div className="flex shrink-0 items-center gap-2">{home && <QuietButton data-help-entry onClick={openHelp}><CircleHelp className="size-4" aria-hidden="true" />Help</QuietButton>}{showGallery && <>
              <IconButton label={selecting ? 'Done selecting' : 'Select media'} onClick={() => { setSelecting(!selecting); setSelectionError(''); if (selecting) setSelected(new Set()) }}>{selecting ? <Check className="size-4" /> : <CopyCheck className="size-4" />}</IconButton>
