...
// rather than adopting whatever the gallery or search is showing.
function storedReelsFilters(): Filters | null {
  try {
    const stored = localStorage.getItem(reelsFilterKey)
    const parsed: unknown = stored ? JSON.parse(stored) : null
    return parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed as Filters : null
  } catch { return null }
}

// Reels can freely rewrite `filters` (forced video type, its own library/folder scope,
// its own sort) while the person browses there. Returning to Library or Search must
// never inherit that: restore the last filters actually used in those modes instead of
// patching the Reels-tainted value, so browsing Reels never moves the Library's place.
function libraryFilters(value: Filters, from: Section, browseMemory: Filters): Filters {
  return from === 'reels' ? browseMemory : value
}

function searchFilters(value: Filters, from: Section, browseMemory: Filters): Filters {
  return from === 'reels' ? browseMemory : value
}
...
export function App() {
  ...
  const [folderMetadataOpen, setFolderMetadataOpen] = useState(false)
  ...
  const reelsMemory = useRef<Filters | null>(storedReelsFilters())
  const browseMemory = useRef<Filters>(filters)
  ...
  function navigateSection(target: Section) {
    ...
    else if (target === 'reels') apply(reelsMemory.current ?? reelsFilters(filters), target)
    else if (target === 'library') apply(libraryFilters(filters, section, browseMemory.current), target)
    else if (target === 'search') apply(searchFilters(filters, section, browseMemory.current), target)
  ...
  const galleryTitle = section === 'reels' ? 'Reels' : section === 'collections' ? 'Collections' : filters.q ? `Results for “${filters.q}”` : folders.data?.current.name ?? library?.name ?? 'Library'
  ...
  return <div className="flex h-dvh min-h-0 flex-col overflow-hidden bg-canvas text-ink">
    ...
    {section !== 'reels' && <SearchHeader value={search} onChange={setSearch} onHome={() => apply({})} onFilters={() => setFilterOpen(true)} onSearch={submitSearch} onClear={clearSearch} searchRequested={section === 'search'} />}
    <div className="flex min-h-0 flex-1"><aside className="hidden w-56 shrink-0 border-r border-line p-3 md:block"><nav aria-label="Primary navigation" className="space-y-2">{navItems.map(item => { const Icon = item.icon; const current = section === item.id; return <QuietButton key={item.id} aria-current={current ? 'page' : undefined} className={`w-full justify-start ${current ? 'border-transparent bg-surface text-accent' : 'border-transparent'}`} onClick={() => navigateSection(item.id as Section)}><Icon className="size-4" />{item.label}</QuietButton> })}</nav><div className="mt-8 border-t border-line pt-4"><p className="mb-2 px-3 text-xs font-semibold uppercase tracking-wider text-muted">Libraries</p>{libraries.data?.map(item => <QuietButton key={item.id} className={`mb-1 w-full justify-start border-transparent ${filters.libraryId === item.id && section === 'library' ? 'bg-surface' : ''}`} onClick={() => visitLibrary(item)}><FolderOpen className="size-4" /><span className="truncate">{item.name}</span></QuietButton>)}</div></aside>
      <main key={`${section}:${queryString(filters)}`} id="collection" className={`motion-page flex min-w-0 flex-1 flex-col ${section === 'reels' ? 'pb-0' : 'pb-20 md:pb-0'}`} tabIndex={-1}>
        {section === 'settings' ? <div className="overflow-auto"><div className="mx-auto max-w-5xl p-5"><MetadataExchange dislikes /></div><ThemePicker value={theme} onChange={setTheme} /><section className="mx-auto grid max-w-5xl gap-4 p-5 sm:grid-cols-3"><div className="rounded-3xl border border-line bg-surface p-5 sm:col-span-3"><p className="text-sm font-medium text-accent">Guide</p><h2 className="mt-1 text-2xl font-semibold">Help and shortcuts</h2><p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted">Quick controls for browsing large collections without leaving the keyboard or touch surface.</p></div>{helpCards.map(card => { const Icon = card.icon; return <article key={card.title} className="rounded-3xl border border-line bg-surface p-5"><span className="mb-4 flex size-12 items-center justify-center rounded-2xl bg-canvas text-accent"><Icon className="size-6" /></span><h3 className="text-base font-semibold">{card.title}</h3><p className="mt-2 text-sm leading-relaxed text-muted">{card.body}</p></article> })}</section></div> : <>
          ...
          {section === 'library' && filters.folderId && <QuietButton className="w-fit" onClick={() => setFolderMetadataOpen(true)}>Import folder metadata</QuietButton>}
    ...
    <Modal open={folderMetadataOpen} onOpenChange={setFolderMetadataOpen} title="Import folder metadata" description="Merge EXIF/XMP tags from every indexed photo and video directly in this folder, regardless of the current filters. Originals stay unchanged." sheet><div className="overflow-auto p-5"><MetadataExchange key={filters.folderId} filters={{ libraryId: filters.libraryId, folderId: filters.folderId }} exports={false} /></div></Modal>
