...
import { Bookmark, ChevronRight, CircleHelp, Clapperboard, FolderOpen, Images, RefreshCw, Search, Settings, Tag, X } from 'lucide-react'
...
import { TaskQueue } from '../features/status/TaskQueue'
...
export function App() {
  ...
  return <div className="flex h-dvh min-h-0 flex-col overflow-hidden bg-canvas text-ink">
      ...
      <main key={`${section}:${queryString(filters)}`} id="collection" className="motion-page flex min-h-0 min-w-0 flex-1 flex-col" tabIndex={-1}>
        {section === 'settings' ? <div className="overflow-auto" data-scroll-restore><ThemePicker value={theme} onChange={setTheme} /><AlbumCoverSettings /><SettingsSection title="Random media URL" description="Generate a reusable random-media link using your current browsing filters."><RandomUrls filters={filters} /></SettingsSection><SettingsSection title="Metadata exchange" description="Download metadata and disliked paths from your library."><MetadataExchange dislikes heading={false} /></SettingsSection><HiddenFolders /><SettingsSection title="Help" description="See what each button, gesture and keyboard shortcut does."><QuietLink data-help-entry href={`?${helpParams}`} className="w-fit" onClick={event => { event.preventDefault(); openHelp() }}><CircleHelp className="size-4" aria-hidden="true" />Open the Help guide</QuietLink></SettingsSection><SettingsSection title="About" description="Version and project information for this Luma installation."><div className="space-y-2 text-sm"><p><span className="font-medium">Luma</span> <span className="text-muted">v{packageJson.version}</span></p><p className="text-muted">Created by Sameer Trivedi.</p><QuietLink href="https://github.com/sameert89/luma" target="_blank" rel="noreferrer">github.com/sameert89/luma</QuietLink></div></SettingsSection></div> : section === 'help' ? <Help onBack={closeHelp} /> : <>
          {section !== 'reels' && <div className="shrink-0 space-y-2 px-4 pb-2 pt-3 sm:px-6 sm:pb-3 sm:pt-4"><div className="flex min-w-0 items-center gap-2">
            ...
            <div className="flex shrink-0 items-center gap-2">{home && <>{libraries.data?.some(item => item.rootFolderId) && <QuietButton onClick={() => setIndexingOpen(true)}><RefreshCw className="size-4" aria-hidden="true" />Indexing status</QuietButton>}<QuietButton data-help-entry onClick={openHelp}><CircleHelp className="size-4" aria-hidden="true" />Help</QuietButton></>}{showGallery && <>
    ...
    <Modal open={indexingOpen} onOpenChange={setIndexingOpen} title="Indexing status" description="Review indexing progress and controls for your libraries." sheet><div className="space-y-5 overflow-auto p-5"><TaskQueue />{libraries.data?.filter(item => item.rootFolderId).map(item => <section key={item.id} aria-labelledby={`indexing-library-${item.id}`}><h2 id={`indexing-library-${item.id}`} className="font-semibold">{item.name}</h2><ScanControls libraryId={item.id} /></section>)}</div></Modal>
